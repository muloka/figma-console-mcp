		...
		server.tool(
			...
			async ({ variableId, modeId, value }) => {
				try {
					...
					// Invalidate variables cache so next read sees the change
		...
		server.tool(
			...
			async ({
				...
				try {
					...
					// Invalidate variables cache so next read sees the new variable
					this.variablesCache.clear();
		...
		server.tool(
			...
			async ({ variableId }) => {
				try {
					...
					// Invalidate variables cache so next read reflects the deletion
					this.variablesCache.clear();
...
return {
  ...
  results
					...
					// Invalidate variables cache so next read sees updated values
					this.variablesCache.clear();
...
return {
  ...
  results
					...
					// Invalidate variables cache so next read sees the new tokens
					this.variablesCache.clear();
